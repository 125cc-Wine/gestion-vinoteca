...
function ConsultarTab({ empresa }: { empresa: string }) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      setResultados(all.filter(p => normalize(`${p.nombre} ${p.bodega || ''} ${p.varietal || ''} ${p.sku || ''}`).includes(q)).slice(0, 10))
      setBuscando(false)
    }, 250)
  }, [search, empresa])
...
function RegistrarAnadaView({ empresa, onGuardado }: { empresa: string; onGuardado: () => void }) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      const q = normalize(search)
      setResultados(all.filter(p => normalize(`${p.nombre} ${p.bodega || ''} ${p.varietal || ''} ${p.sku || ''}`).includes(q)).slice(0, 8))
      setBuscando(false)
    }, 250)
  }, [search, empresa])
